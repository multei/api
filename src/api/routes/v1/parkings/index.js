...
router.get('/:car_plate', async function (req, res, next) {
  ...
  const handleSuccess = response => {
    ...
    debug('Success on listing data: %o', response)

    if(response.length === 0) {
      res.status(404).send(`No parking found with this plate. Received: ${car_plate}`);
      next();
    }
    res.status(200).json(response);
  };
  ...
  const handleError = error => {
    debug('Error: %o', error)
    next(createError(500, 'Can not retrieve parkings'))
  };
 ...
 */
router.delete('/:car_plate', bodyParser.json(), async (req, res, next) => {
